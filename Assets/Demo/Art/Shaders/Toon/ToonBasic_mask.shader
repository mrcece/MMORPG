Shader "Shader Forge/ToonBasic_mask" {
    Properties {	
    	_SceneColor ("SceneColor", Range(0, 2)) = 1
...
////// Lighting:
////// Emissive:
       			float3 node_5977 = viewDirection.rgb;
                ...
                float3 emissive = (((lerp((node_3582_if_leA*_Diffuse_var.rgb)+(node_3582_if_leB*node_6599),_Diffuse_var.rgb,node_3582_if_leA*node_3582_if_leB)*clamp(node_723,0.6,1))+(step(_ShadowThreshlold,node_8801)*((_SpecularBrightness*_SpecularBrightness*_MASK_var.g)*step(_SpecularRange,saturate((saturate(max(0,dot(halfDirection,i.normalDir)))*(_MASK_var.b*_SpecularPower)))))))*_SceneColor);
